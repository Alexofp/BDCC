...
func getName() -> String:
	return "The Daily Drip"

func getDesc(_isFull:bool) -> String:
	if(_isFull):
		return "Get sold on a slave auction to a lovely couple that runs a cafe in dire need of milk and cum."
	return "Get sold on a slave auction to a lovely couple that runs a cafe in dire need of milk and cum."
...
func getKinks() -> Array:
	return [
		"Soft bad end",
		"Player is a sub",
		"Milking, lots of milking (pc is target)",
		"Sex/Pegging/Breeding",
	]
...
func getStatus() -> int:
	if(GM.main.PSH.didComplete("MilkCafe")):
		return ContentBoardStatus.Completed
	if(GM.main.isInSpecificPlayerSlavery("MilkCafe")):

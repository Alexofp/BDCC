...
func defaultStruggle(_pc, _minigame, response):
	...
	if !_pc.hasBlockedHands() && !_pc.isBitingBlocked():
		...
		response.text = "{user.name} uses {user.his} hands and mouth to try and take off the {item.name}."
	elif !_pc.hasBlockedHands():
		response.text = "{user.name} can't use {user.his} mouth but {user.he} can just about reach the {item.name} with {user.his} hands. {user.name} is tugging on the restrant, trying to slip it off."
	elif !_pc.isBitingBlocked():
		response.use.append("mouth")
		response.text = "{user.name} is trying to wiggle the {item.name} off. Not being able to use hands really sucks but using {user.his} mouth instead helps {user.him} to keep them still."
	return response

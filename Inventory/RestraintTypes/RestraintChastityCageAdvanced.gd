...
func sucessStruggleLocked(_pc, _minigame, response):
	response.text = "Nothing despite best efforts, there's no way to escape as long as the thing is locked."
	return response


func afterPick(_pc, _minigame, response):
	if failChance(_pc, 20):
		response.text += " Opps, {user.name}'s chastity cage BEEPS.. And then suddenly [b]shocks[/b] {user.his} {user.penis}!"

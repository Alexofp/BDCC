...
func defaultStruggle(_pc, _minigame, response):
	if !_pc.hasBoundArms() && !_pc.hasBlockedHands():
		...
		response.text = "{user.name} tries to bite down on the ring in {user.his} mouth although the rubber makes it very tough"
...
func fatalFailStruggle(_pc, _minigame, response):
	if response.use.has("hands"):
		...
		response.text += " but the ring just stuck in {user.his} mouth."
...
func afterStruggle(_pc, _minigame, response):
	if(failChance(_pc, 25)):
		response.text += " The ring in {user.his} mouth makes {user.him} drool a lot."

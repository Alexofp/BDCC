...
func _run():
	...
	if(state == ""):
		...
		HK_Util.addUnresistableHypnoButtons(self, "moving_towards", ["Head out", "Get moving", "Fetch"])
	...
	if(state == "moving_towards" || state == "moving_back"):
		...
		HK_Util.addHypnoButtons(self, "moving_towards", "resist", ["Fetch", "Move"], ["Stop", "Resist"], 1)
	...
	if (state == "resist"):
		...
		HK_Util.addHypnoButtons(self, "moving_towards", "resist_2", ["Fetch", "Move"], ["Stop", "Resist"], 1)
	if (state == "resist_2"):
		removeCharacter(npcID)
...
func _react(_action: String, _args):
	...
	if(_action == "return"):
		aimCamera(startLocation)
		GM.pc.setLocation(startLocation)
		GM.pc.addSkillExperience(HK_Skill.HypnosisSkill, 5)

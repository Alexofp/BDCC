...
func _run():
	if(state == ""):
		...
		saynn("[say=HK_Vion]Bondage is such an interesting, multifaceted topic. What is it about it that interests you? Is it the feeling of restriction, of vulnerability, of powerlessness? Or is it the snug restraints that allow you not to be in control, to simply submit?[/say]")
	...
	if(state == "after_intro"):
		playAnimation(StageScene.Duo, "sit", {npc="HK_Vion", npcAction="sit"})
		saynn("[say=HK_Vion]I'll start by locking these special cuffs on you. Until I say otherwise, you won't be able to take them off, no matter what you do. Understand?[/say]")
		...
		saynn("[say=HK_Vion]And there's the second one. They feel wonderful, don't they? Snug, tight, restrictive and secure, but also just the right size for you and perfectly comfortable.[/say]")

		saynn("He pushes your arms behind you and clamps your wrists together.")

		saynn("[say=HK_Vion]There, out of sight and out of mind. Inescapable. Not just restrained, though. Feel them go rigid as my commands wrap around them, taking control of them, entwining with them... Completely trapped, powerless in my grasp. No struggling allowed. No escape possible.[/say]")
	...
	if(state == "after_wrist_cuffs"):
		playAnimation(StageScene.Duo, "sit", {npc="HK_Vion", npcAction="sit"})
		saynn("[say=HK_Vion]Now let's take care of your legs. Keep nice and still now...[/say]")
		...
		saynn("[say=HK_Vion]Excellent, just as secure. Do feel free to [i]try[/i] and struggle if you'd like, though...[/say]")
	...
	if(state == "after_ankle_cuffs"):
		playAnimation(StageScene.Duo, "kneel", {npc="HK_Vion", npcAction="sit"})
		saynn("[say=HK_Vion]The best part about being restrained by the power of your own mind, though? Its how convenient it can be. [b]Kneel[/b] for me.[/say]")
		...
		saynn("[say=HK_Vion]Let's strap you down a bit more, shall we? First let's tie those limbs down...[/say]")

		saynn("You feel his hands brush up against your wrists and ankles as he secures restraints to them.")

		saynn("[say=HK_Vion]That's better. Even if you could move without permission you'd get nowhere now. Now let's do something about this collar, hm?[/say]")
		...
		saynn("[say=HK_Vion]I'm locking your head and neck in place. That's right, no looking back now~[/say]")
	...
	if(state == "after_anchoring"):
		...
		if(GM.pc.hasTail()):
			saynn("[say=HK_Vion]One last thing, just in case you fancy yourself an escape artist. Let's tie this tail down, nice and secure.[/say]")

			saynn("He pulls your tail taut, then forces it to lie flat against the ground.")

		saynn("[say=HK_Vion]That should do, I think. Remember, not a peep and keep your eyes closed.[/say]")
	...
	if(state == "wait"):
		...
		saynn("[say=HK_Vion]Hmm hm hmmm~[/say]")

		saynn("And yet, you continue to wait...")

		saynn("[say=HK_Vion]And done! I'm no artist, but you do make a great model. [i]Very[/i] good at holding a pose! Maybe I'll show you the sketch later, when you're more... present.[/say]")

		saynn("Again, you hear the sofa creak as Vion stands up and circles back in front of you.")

		saynn("[say=HK_Vion]Would you like to get up now? Just nod if you would.[/say]")

		saynn("With the greatest of struggles, you manage a barely perceptible nod.")
		
		saynn("[say=HK_Vion]Good {pc.boy}. The cuffs stay on for a bit longer, though~[/say]")

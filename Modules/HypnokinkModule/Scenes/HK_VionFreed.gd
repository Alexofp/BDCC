...
func _run():
	if(state == ""):
		...
		saynn("[say=HK_Vion]What are you up to?[/say]")
	...
	if(state == "hack_fail"):
		...
		saynn("[say=HK_Vion]Everything okay?[/say]")
	...
	if(state == "hack_succeed"):
		...
		saynn("[say=HK_Vion]Did you just-[/say]")
		saynn("[say=pc]Yup. You can thank me later.[/say]")
		saynn("[say=HK_Vion]...I can thank you [i]now[/i].[/say]")
		saynn("The almost animalistic fierceness of that statement catches you somewhat off guard.")
		saynn("[say=HK_Vion]Unless you aren't interested, in which case I'd suggest you vacate this cell for the next few hours.[/say]")
	...
	if(state == "bail"):
		...
		saynn("[say=HK_Vion]Oh, I will.[/say]")
		...
		saynn("[say=HK_Vion]Hey! You up for a fuck?[/say]")
	...
	if(state == "ride_it_out"):
		...
		saynn("[say=HK_Vion]And [b]kneel.[/b][/say]")
		
		addButton("Kneel", "", "sex_1")

	if(state == "sex_1"):
		playAnimation(StageScene.SexOral, "fast", {npc="pc", pc="HK_Vion", bodyState={exposedCrotch=true, hard=true}})
	...
	if(state == "sex_2"):
		playAnimation(StageScene.SexStanding, "fast", {npc="pc", pc="HK_Vion", bodyState={naked=true, hard=true}, npcBodyState={exposedCrotch=true}})	
	...
	if(state == "sex_3"):
		playAnimation(StageScene.SexMissionary, "sex", {npc="pc", pc="HK_Vion", bodyState={naked=true, hard=true}, npcBodyState={exposedCrotch=true}})
	...
	if(state == "sex_4"):
		playAnimation(StageScene.SexAllFours, "fastflop", {npc="pc", pc="HK_Vion", bodyState={naked=true, hard=true}, npcBodyState={exposedCrotch=true}})
	...
	if(state == "sex_after"):
		playAnimation(StageScene.Sleeping, "sleep", {npc="HK_Vion", pc="pc", bodyState={naked=true}, npcBodyState={naked=true}})
		saynn("A few hours later...")
		
		saynn("[say=HK_Vion]Hey. Hey {pc.name}.[/say]")
		saynn("[say=pc]Mmh?[/say]")
		saynn("[say=HK_Vion]That wasn't too intense, I hope? Are you still with me?[/say]")
		saynn("You manage to slur out a response.")
		saynn("[say=pc]Nnh. I'm... I'm fi-ine.[/say]")
		saynn("[say=HK_Vion]Well. That's good. Hope you don't mind, but I'm going to nap for a while.[/say]")

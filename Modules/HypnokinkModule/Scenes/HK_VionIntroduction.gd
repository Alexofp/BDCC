...
func _run():
	if(state == ""):
		...
		saynn("[say=HK_Vion]If you want one of your own, you'll have to wait until the next time the guards can't be bothered dragging one off for disposal.[/say]")
	...
	if(state == "just_looking"):
		...
		saynn("[say=HK_Vion]Well, feel free to look around. Not much to it, I'm afraid.[/say]")
	...
	if(state == "introduction"):
		...
		saynn("[say=HK_Vion]I go by {HK_Vion.name}. Pleasure. Now, care to clear the door and let me into my parlor?[/say]")
	...
	if(state == "gang_way"):
		...
		saynn("You step aside and watch as {HK_Vion.name} walks inside, then plops down on the bed.")
		
		saynn("[say=HK_Vion]So {pc.name}, do you need anything?[/say]")

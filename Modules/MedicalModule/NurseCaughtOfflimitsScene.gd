...
func _react_scene_end(_tag, _result):
	...
	if(_tag == "guardfight"):
		...
		if(battlestate == "win"):
			...
			if(GM.ES.triggerReact(Trigger.WonCaughtOffLimitsFight, [npcID, CharacterType.Nurse])):
				endScene()
				return
		else:
			if(GM.ES.triggerReact(Trigger.LostCaughtOffLimitsFight, [npcID, CharacterType.Nurse])):
				endScene()
				return
			#setState("if_lost")

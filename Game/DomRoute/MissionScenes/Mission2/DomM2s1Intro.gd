...
func _run():
	if(state == ""):
		...
		addCharacter("kait")
		saynn("You're standing near Kait.. alone for now.")

		saynn("[say=kait]We have the records now![/say]")

		saynn("Kait turns around and begins attaching them to the planning board.")
		...
		saynn("The snow leopard keeps staring at the lists.")

		saynn("[say=kait]So many names.. How do we pick.. We probably don't want any traitors. Do we want murderers though? What about terrorists.. I wanna capture this prison, not destroy it.[/say]")

		saynn("She skims the names.")
		...
		saynn("Kait crosses her arms and then scritches her chin.")

		saynn("[say=kait]The staff too, how do we pick the best targets..[/say]")

		saynn("Suddenly, a familiar voice.")
	...
	if(state == "avy_comes_in"):
		...
		saynn("Avy grabs both lists from the board and starts quickly going over them.")
		...
		addButton("Continue", "See what happens next", "avy_presentation")
	if(state == "avy_presentation"):
		playAnimation(StageScene.Solo, "stand", {pc="avy"})
		saynn("Avy grabs the marker and writes the word 'Captain' on the board. Below it, she writes 'Skar' with smaller letters.")
	...
	if(state == "show_skar"):
		...
		saynn("[say=avy]Calm your tits, sugar. He is the one who protects the office. But the captain isn't always glued to his chair.[/say]")
	...
	if(state == "risha_nova_showcase"):
		...
		saynn("[say=avy]Yes, shut up. The third guard is Nova. We saw her.[/say]")
		...
		saynn("[say=announcer]Our captain just doesn't have much of a choice.[/say]")
	...
	if(state == "alex_eliza_intros"):
		...
		saynn("Avy looks through the list of inmates.. and starts writing down a certain name..")
	...
	if(state == "ferri_reveal"):
		...
		saynn("Avy shows the name 'Ferri'.")

		saynn("[say=avy]Name is Ferri. A red. A hybrid of two species apparently. A dragon and a cat. Huh, I'd watch that.[/say]")

		saynn("[say=kait]Of course you would. And why would we need this 'Ferri'?[/say]")

		saynn("[say=avy]It says here that she is a chemist. Did some nasty stuff.[/say]")
		...
		saynn("[say=kait]We're not terrorists, Avy. I hate this prison but destroying it would also kill us.[/say]")

		saynn("[say=avy]Who said anything about the whole prison? Let me tell you what will happen when we start our last attack. The whole prison is gonna go into a full fucking lockdown. All the doors are gonna be locked and bolted shut. How do you plan on getting to the captain?[/say]")

		saynn("[say=kait]Well..[/say]")

		saynn("[say=avy]That's right, we're gonna break this slut next. She is a total doormat anyway, I fought her in the arena.[/say]")

		saynn("[say=announcer]Sounds like a good plan. I will keep the hideout ready for her arrival.[/say]")

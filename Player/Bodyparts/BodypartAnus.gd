...
func getTransformMorphMessage(_context:Dictionary) -> String:
	...
	if(hasWomb):
		if(hasVag):
			return "{npc.YouHe} {npc.youHeVerb('feel')} a tingling at {npc.yourHis} rear, and suddenly, {npc.yourHis} anus begins to change. The shape shifts, becoming more pronounced and adorned with new textures. The change is much bigger than it looks on the outside.. This new tailhole now shares a womb with {npc.yourHis} pussy, intertwining their functions."
		return "{npc.YouHe} {npc.youHeVerb('feel')} a tingling at {npc.yourHis} rear, and suddenly, {npc.yourHis} anus begins to change. The shape shifts, becoming more pronounced and adorned with new textures. The change is much bigger than it looks on the outside.. A womb forms inside {npc.youHim}, connected to your tailhole."
	return "{npc.YouHe} {npc.youHeVerb('feel')} a tingling at {npc.yourHis} rear, and suddenly, {npc.yourHis} anus begins to change. The shape shifts, becoming more pronounced and adorned with new textures. The change is significant, yet it kept {npc.yourHis} current tightness. {npc.YouHe} now {npc.youHaveHeHas} "+getAVulgarName()+"."

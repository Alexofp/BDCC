...
func _run():
	...
	if(state == "stepinside"):
		...
		say("The wolf enters the room as well, making you go further before locking the door behind and guiding his attention at you. {intro_detective.say('I can grab a bottle of water if you want')}. He then points at a far chair and adds {intro_detective.say('Take a sit please')}\n\n")
	...
	if(state == "sitdown"):
		...
		say("{intro_detective.say('Jake Anderson, IIPF, threat assessment division. Can you state your name for me? I will also need some bio information, we can’t seem to find you in our database')}. The guy pulls out some kind of tablet and turns it on. You can hear a subtle hum as a fancy blue hologram menu appears. He then takes out a pen and starts to interact with it, sending blue hologram particles to fly in the process.\n\n")
	...
	if(state == "donecreating"):
		say("The wolf nods. {intro_detective.say('Alright. You look slightly lost there. Long story short is that you failed a mindtest procedure. Information that was gathered is enough to link you to the crime that we\\'ve been trying to solve. I will be blunt. It’s enough to put you in jail for a while.')} He takes a short pause and watches your reaction. {intro_detective.say('And my job here is to figure out why you did it')}\n\n")
	...
	if(state == "wantlawyer"):
		say("{pc.say('I won’t speak without a lawyer')} you say. You saw all those videos about how damaging it can be to say anything to the police so you cross your arms and put on a mean face.\n\n")
		
		say("Jake sighs. {intro_detective.say('The amount of evidence against you is very high, you’re not doing yourself any favors by denying the facts')}. His strict glare drills you. {intro_detective.say('Do you wanna be portrayed as a monter? Fine by me. I’m just trying to give you a chance to redeem yourself and you’re just sitting there, thinking your safe corner will protect you')}\n\n")
		
		addButton("Admit to", "Maybe you actually did it..", "admit")
		addButton("No", "Stay silent", "staysilent")
		
	if(state == "staysilent"):
		say("{pc.say('Just give me a lawyer, I’m done talking to you')}\n\n")
		
		say("Jake stands up and takes a step towards you. {intro_detective.say('You know what, I’m done playing games')}. He growls and grabs your arm, trying to wrench it behind your back. It catches you off guard but you manage to shove him away.\n\n")
		
		say("But then you realize that his goal wasn’t to wrench your arms. You notice that he holds a used drug injector. You look at your arm and you see a little hole that the needle left. Anger quickly fades, replaced by the strange soothing feeling, your limbs lose their strength. {pc.say('What was that..')}. The officer sees that the drug has kicked in and sits back down.\n\n")
		
		say("{intro_detective.say('It’s a sedative. And also a little thing called Truth serum')}. Jake relaxes in his chair. {intro_detective.say('Let’s try again. Why did you do it?')}\n\n")
	...
	if(state == "didntdoit"):
		say("{pc.say('What do you mean, I didn’t do anything')}. Your mind is still hazy, it's hard to contrentate and the officer's words begin to settle panic into you.\n\n")
		
		say("{intro_detective.say('You don’t get it, we’re long past that.')} Jake tilts his head slightly and puts away his tablet. {intro_detective.say('We have evidence and your best option right now is to explain exactly what happened')}. Jake leans towards you and tries to keep eye contact while you look away, shivering. {intro_detective.say('Otherwise, as you know, people always assume the worst. Do you wanna be painted as a monster by them? I\\'m here to help you. It happens. Anyone can make a mistake. But we can work through it')}\n\n")
	...
	if(state == "ikilled"):
		say("{pc.say('I.. I found a gun. Well, not found.. but you know.. Someone gave it to me')}. You place your fist on the table and focus your glance on it. {pc.say('And I shot a guy with it..')}\n\n")
		
		say("Officer nods slightly, he moves his chair even closer to you. {intro_detective.say('And what made you do it?')}\n\n")
	...
	if(state == "reasonmoney"):
		say("{pc.say('I had to, I had no credits left. Another week and I would be out on the streets. I got this offer. And I had to do it. I truly had no other choice..')}\n\n")
		
		say("You tell the officer the whole story, it took a while. But you do feel better.\n\n")
		
		say("Jake stands up and takes something from his belt. {intro_detective.say('Alright. I’m afraid I will have to cuff you. Just a safety thing')}. You recoil back slightly but seeing as there is no better choice, you reach your hands forward while he puts tight police cuffs around them. {intro_detective.say('I will be back in a few minutes and we will figure out what to do with you')}. Jake exits the room, leaving you alone.\n\n")
		
		addButton("Wait", "What else is there to do", "saidwhy")
		
	if(state == "reasonrevenge"):
		say("You stress your fist. {pc.say('He had it coming, that fucker.. Betraying piece of shit')}\n\n")
	...
	if(state == "waitmore"):
		...
		say("{intro_detective.say('I’m here to tell you that the court of Armada has found you guilty of first-degree manslaughter. You will be committed to the department of corrections.')}\n\n")
		
		say("{pc.say('Huh, what do you mean? Shouldn’t there be a trial? You’re not a judge, hey, HEY!')}\n\n")
		
		say("The officer stops reading and guides his attention towards you. {intro_detective.say('This is a trial. The judges made their decision. Soon you will be repositioned into the place called Broken Dream Correctional Center where you will be serving your punishment. Your sentence is..')}. He looks at the tablet again. {intro_detective.say('Indefinite')}. He gestures to the two officers. {intro_detective.say('Put her in a holding cell for now, the ship is gonna do FTL jump soon')}\n\n")
	...
	if(state == "gocell"):
		...
		say("{sayMale('There is no escape. Unless you know how to breathe in the void. Hah')}\n\n")
		
		say("{pc.say('Just.. shut up')}\n\n")
		
		say("{sayMale('You better get ready to spread these legs of yours. I heard many stories. You’re fucked. In more ways than one. Pfff')}\n\n")
		...
		say("{sayMale('Here')}. You turn and look out of the window. Between all the asteroids you see a huge rock, the biggest one, floating in the void with a space station seemingly glued to it. The station looks small from this distance but you do see a few levels to it. {sayMale('That’s where you will spent the rest of your life')}\n\n")
		...
		say("The guard says {sayMale('We’re here. Now get the fuck out, they’re waiting for you')} as he opens the shuttle door and watches you. {sayMale('Oh and. The cuffs.')} He stops you to quickly uncuff your hands before shoving you out. {sayMale('They will give you better ones, don’t worry sweetie')}\n\n")
